...
export function Document({
  ...
  // Function to handle adding a new slide with style selection
  const handleAddPage = (pageType: SlideType, slideStyle?: string, addPosition?: 'left' | 'right') => {
    // For intro and outro slides, use the slideStyle if provided
    const newSlide = getDefaultSlideOfType(pageType, slideStyle);
    
    // Add slide based on which button was clicked
    if (addPosition === 'left') {
      prepend(newSlide);
    ...
    <div className=" flex flex-row justify-center w-full">
      <Carousel
        ...
        style={{
          ...
          transformOrigin: "top",
          minWidth: `${400 + SLIDE_PADDING_X}px`,
        ...
        >
          ...
          >
            <NewPage
              size={SIZE}
              className="px-2"
              handleAddPage={(pageType, slideStyle) => handleAddPage(pageType, slideStyle, 'left')}
          ...
          >
            <NewPage
              size={SIZE}
              className="px-2"
              handleAddPage={(pageType, slideStyle) => handleAddPage(pageType, slideStyle, 'right')}
              isSideButton={newPageAsSideButton}
            />
          </CarouselItem>
        </CarouselContent>
